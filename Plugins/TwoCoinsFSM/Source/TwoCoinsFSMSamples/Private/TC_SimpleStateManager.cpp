...
void UTC_SimpleStateManager::TickComponent(float DeltaTime,
                                           ELevelTick TickType,
...
void UTC_SimpleStateManager::StateTransitionTo(const FName stateNameToTransitTo)
{
	...
	UE_LOG(LogTwoCoinsFSMSimple,
	       Warning,
	       TEXT("FSM CurrentState: %s"),
	       *fsm->GetCurrentState()->GetStateName().ToString());
...
void UTC_SimpleStateManager::BindFSM_Internal()
{
	if (!fsm)
	{
		return;
	}
	if (fsm->OnFSMStateTransitionedDelegate.IsBound())
	{
...
void UTC_SimpleStateManager::OnFsmStateTransitioned(const FName transitionName,
                                                    const FName oldStateName,
                                                    const FName newStateName) const
{
	UE_LOG(LogTwoCoinsFSMSimple,
	       Warning,
	       TEXT("FSM Transitioned: %s, [%s] --> [%s] (:: %s) | %hs"),
